...
class _CompletedTabState extends State<CompletedTab> {
  ...
  Future<void> printTransaction(Map<String, dynamic> transaction) async {
    try {
      // Check print status - 1: print original+copy, 2: print only copy
      int printStatus =
          int.tryParse(transaction['print_status']?.toString() ?? '1') ?? 1;

      if (printStatus == 1) {
        _showMessage('Printing receipts (original and copy)...');
      } else {
        _showMessage('Printing copy receipt...');
      }

      // Try Sunmi printer first if available
      if (_isSunmiAvailable) {
        await _printWithSunmi(transaction, printStatus);
      } else {
        // Fall back to Bluetooth printer
        await _printWithBluetooth(transaction, printStatus);
      }
    } catch (e) {
      _showMessage('Error printing receipts: $e');
    }
  }

  // Add a method to update the print status via API
  Future<void> _updatePrintStatus(String transactionId) async {
    try {
      print('Updating print status for transaction: $transactionId');
      final response = await http.post(
        Uri.parse('${AppConfig.baseUrl}/api/transaction/command/print_status'),
        headers: {'Content-Type': 'application/json'},
        body: json.encode({'trans_id': transactionId}),
      );
      print(response.body);

      if (response.statusCode == 200) {
        final responseData = json.decode(response.body);
        print('Print status update response: $responseData');

        if (responseData['success'] == 200 || responseData['success'] == 200) {
          print('Print status updated successfully');
        } else {
          print(
              'Failed to update print status: ${responseData['message'] ?? "Unknown error"}');
        }
      } else {
        print('Error updating print status: HTTP ${response.statusCode}');
      }
    } catch (e) {
      print('Exception when updating print status: $e');
    }
  }

  // Print using Sunmi printer with print status handling
  Future<void> _printWithSunmi(
      Map<String, dynamic> transaction, int printStatus) async {
    setState(() {
    ...
    try {
      if (printStatus == 1) {
        // Print original receipt first
        await _printSingleSunmiReceipt(transaction, "ORIGINAL");

        // Notify user about the delay
        _showMessage(
            'Printed original receipt. Waiting 20 seconds before printing copy...');

        // Add a 20-second delay between prints
        await Future.delayed(Duration(seconds: 20));

        // Print copy receipt
        await _printSingleSunmiReceipt(transaction, "COPY");

        _showMessage('Both receipts printed successfully using Sunmi printer');

        // Update print status after successful printing
        if (transaction['trans_id'] != null) {
          await _updatePrintStatus(transaction['trans_id'].toString() ?? "2");
        }
      } else {
        // Print only copy receipt
        await _printSingleSunmiReceipt(transaction, "COPY");
        _showMessage('Copy receipt printed successfully using Sunmi printer');
      }
  ...
  // Helper method to print a single receipt with Sunmi printer
  Future<void> _printSingleSunmiReceipt(
      Map<String, dynamic> transaction, String receiptType) async {
    await SunmiPrinter.initPrinter();

    // Print the company logo if available
    if (transaction['logo'] != null && transaction['logo'].isNotEmpty) {
      try {
        print('Logo URL found in transaction: ${transaction['logo']}');
        final logoBytes = await _downloadLogo(transaction['logo']);
        if (logoBytes != null) {
          print('Printing logo on Sunmi printer');
          await SunmiPrinter.setAlignment(SunmiPrintAlign.CENTER);
          await SunmiPrinter.printImage(logoBytes);
          await SunmiPrinter.lineWrap(2);
        } else {
          print('Logo bytes were null, skipping logo print');
        }
      } catch (e) {
        print('Error printing logo on Sunmi: $e');
      }
    } else {
      print('No logo URL found in transaction data');
    }
    ...
    await SunmiPrinter.printText("Card type: ${transaction['card_type_name']}",
        style: SunmiTextStyle(
          fontSize: 30,
        ));
    await SunmiPrinter.lineWrap(7); // Add one line of space
    ...
    // Print receipt type (ORIGINAL or COPY)
    ...
    await SunmiPrinter.lineWrap(7); // Add spacing after separator

    await SunmiPrinter.cutPaper();

    // Print an additional separator after cutting the paper for better visual distinction
    await SunmiPrinter.printText("==================================",
        style: SunmiTextStyle(
          fontSize: 30,
          align: SunmiPrintAlign.CENTER,
  ...
  // Print using Bluetooth thermal printer with print status handling
  Future<void> _printWithBluetooth(
    ...
    try {
      if (printStatus == 1) {
        // Print original receipt first
        await _printSingleBluetoothReceipt(transaction, "ORIGINAL");
        ...
        // Print copy receipt
        await _printSingleBluetoothReceipt(transaction, "COPY");

        _showMessage(
            'Both receipts printed successfully using Bluetooth printer');

        // Update print status after successful printing
        if (transaction['trans_id'] != null) {
          await _updatePrintStatus(transaction['trans_id'].toString() ?? "0");
        }
      } else {
        // Print only copy receipt
        await _printSingleBluetoothReceipt(transaction, "COPY");
        _showMessage(
            'Copy receipt printed successfully using Bluetooth printer');
      }
  ...
  // Helper method to print a single receipt with Bluetooth printer
  Future<void> _printSingleBluetoothReceipt(
      Map<String, dynamic> transaction, String receiptType) async {
    // Logo printing code removed completely
    ...
    bluetooth.printCustom("Card type: ${transaction['card_type_name']}", 1, 0);
    bluetooth.printCustom(
        "Smart card: ${transaction['card_number'] ?? 'Unknown'}", 1, 0);
    bluetooth.printNewLine();

    // Print receipt type (ORIGINAL or COPY)
    bluetooth.printCustom("-------------$receiptType-------------", 1, 1);
    bluetooth.printNewLine();
    ...
    // Print an additional separator for better distinction between receipts
    if (receiptType == "ORIGINAL") {
      // bluetooth.printCustom("END OF ORIGINAL RECEIPT", 1, 1);
      bluetooth.printCustom("==================================", 1, 1);
    } else {
      // bluetooth.printCustom("END OF COPY RECEIPT", 1, 1);
      bluetooth.printCustom("==================================", 1, 1);
    }
  ...
  Future<void> fetchCompletedTransactions() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // Print first transaction logo URL for debugging
        if (responseData['success'] == 200 &&
            responseData['data'] != null &&
            responseData['data'].isNotEmpty) {
          print(
              "First transaction logo URL: ${responseData['data'][0]['logo']}");
        }
  ...
  // Helper method to download a logo from URL and return as bytes
  Future<Uint8List?> _downloadLogo(String logoUrl) async {
    print('Attempting to download logo from: $logoUrl');
    try {
      final response = await http.get(Uri.parse(logoUrl));
      if (response.statusCode == 200) {
        print(
            'Logo downloaded successfully, size: ${response.bodyBytes.length} bytes');
        return response.bodyBytes;
      }
      print('Failed to download logo: HTTP status ${response.statusCode}');
      return null;
    } catch (e) {
