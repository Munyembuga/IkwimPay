...
class TakePictureScreenState extends State<TakePictureScreen> {
  ...
  // Character correction maps based on position and format knowledge
  ...
  // Function to apply format-specific corrections to detected text
  String _applyFormatCorrections(String text) {
    ...
    if (text.length == 7 &&
      ...
      for (int i = 0; i < text.length; i++) {
        if (i < 3) {
          ...
          correctedText += _letterCorrections[text[i]] ?? text[i];
        } else if (i >= 3 && i < 6) {
          // Middle 3 should be numbers
          correctedText += _numberCorrections[text[i]] ?? text[i];
        } else {
    ...
    // Format 3: AB123C
        ...
        RegExp(r'^[A-Z0-9]{2}[A-Z0-9]{3}[A-Z0-9]{1}$').hasMatch(text)) {
      for (int i = 0; i < text.length; i++) {
        if (i < 2 || i == 5) {
          // First 2 and last 1 should be letters
          correctedText += _letterCorrections[text[i]] ?? text[i];
        } else {
          // Middle 3 should be numbers
          correctedText += _numberCorrections[text[i]] ?? text[i];
                ...
                children: [
                  ...
                  Expanded(
                    child: ElevatedButton(
                      onPressed: () async {
                        String plateNumber =
                            _manualPlateController.text.trim().toUpperCase();
                        ...
                        // Apply format correction to manual input as well
                        plateNumber = _applyFormatCorrections(plateNumber);

                        // Check if the format is valid
                        if (!_plateRegex.hasMatch(plateNumber)) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                              content: Text(
