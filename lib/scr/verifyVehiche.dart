...
import 'package:ikwimpay/scr/testPlatenumber.dart';
...
class VerifyVehicleScreen extends StatefulWidget {
  ...
  const VerifyVehicleScreen.withoutTransaction({
    Key? key,
    required this.cardId,
...
class _VerifyVehicleScreenState extends State<VerifyVehicleScreen> {
  ...
  Future<void> _scanLicensePlate() async {
    try {
      ...
      // await Navigator.of(context).push(
      //   MaterialPageRoute(
      //     builder: (context) => TakePictureScreenFirst.withoutTransaction(
      //       camera: firstCamera,
      //     ),
      //   ),
      // );
  ...
  Future<void> _startTransaction() async {
    ...
    try {
      ...
      if ((responseData['success'] == 200) || responseData['status'] == '200') {
        // Clear image data from SharedPreferences and local variables
        final prefs = await SharedPreferences.getInstance();
        await prefs.remove('base64Image');
        await prefs.remove('plateNumber');

        // Clear local image data variables
        setState(() {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            ...
            duration: const Duration(
                seconds: 5), // Controls how long the message is visible
          ),
        );
